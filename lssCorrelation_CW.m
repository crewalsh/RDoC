function lssCorrelation_CW(images, rois, settings)
% change made : nanmean --> nan_mean
%               corr --> corrcorr
% CW changes output directories as to not overwrite JB's code 
...
switch settings.fConnType
    case 'seed2voxel'
        outDir = [parentDir '/seed2voxel_correlations_Yeo/'];
        if ~exist(outDir, 'dir')
            mkdir(outDir);
        end
...
%                 fileName = strjoin(separatedFilename, '_');
                % JB CHANGE BECAUSE NO STRSPLIT FUNCTION
                [~, fileName] = fileparts(images{iCond}{jSess});
                if strcmp(fileName(1:3),'4D_')
                    fileName(1:3) = [];
                end
    ...
    case 'roi2roi'
        outDir = [parentDir '/roi2roi_correlations_yeo/'];
        disp(outDir);
        if ~exist(outDir, 'dir')
            mkdir(outDir);
        end
...
threshold = 1/4; % JBP - Find mask values greater than this (mostly for HPC)
...
% Modified JBP dec 2 2015

% need to save original data when outliers are detected
y0 = y;
