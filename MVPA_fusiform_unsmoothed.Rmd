...
clf_acc <- read.csv('data/MVPA/fusiform_unsmoothed/clf_acc.csv', header=FALSE)
best_c <- read.csv('data/MVPA/fusiform_unsmoothed/best_C.csv', header=FALSE)
trial_acc <- read.csv("data/MVPA/fusiform_unsmoothed/all_suj_acc.csv", header = FALSE)
...
On average, we were able to classify faces with 72.7% accuracy (statistically significantly different from chance = 0.33). The classifier was trained on data from an independent FFA localizer. Data was masked using a bilateral fusiform mask from the AAL atlas; from that, the top 100 voxels based on the faces vs objects contrast in the overall subject GLM were selected as features for the classifier. The data used to train the classifier were shifted by 4.5 seconds to account for the hemodynamic delay. 
...
We see significant correlations between high load accuracy and classification probability during high load correct trials and the correct-incorrect difference in high load trials. We also see relationships with omnibus span and low load incorrect trials and the difference between correct and incorrect at low load. 
...
cor.test(individual_trial_averages_probs[["low_incorrect"]]$V6,temp_plot_data$omnibus_span_no_DFR_MRI)
...
There is a significant negative relationship between accuracy at high load and the probability of face classification at low load. There is also a significant relationship between probability at low load incorrect trials and omnibus span and high load accuracy. 
...
cor.test(individual_trial_averages_probs[["low_incorrect"]]$V8,temp_plot_data$XDFR_MRI_ACC_L3)
...
Probability of classification at incorrect high load trial is significantly negatively correlated with high load accuracy during probe and positively with correct - incorrect difference at high load. We also see a correlation between omnibus span and probability of classification at low load incorrect trials.
...
The reason that the template data looks so discretized is because it's only averaging 1 vs 0 (ie is a face vs not a face) over 6 trials (since there's only one "trial" of the average correct template, and there are 6 CV loops, so averaging over those). In contrast, the individual trials have many more to be averaged over, so not as discrete values for each subject. 

### Averaged over time

If we average over time, there are significant relationships between omnibus span and low load incorrect trials and with the difference between correct and incorrect trials at low load and DFR high load acc. 
...
cor.test(averages_from_template[["low_load_correct_diff"]]$V6, temp_plot_data$omnibus_span_no_DFR_MRI)
cor.test(averages_from_template[["low_load_correct_diff"]]$V6, temp_plot_data$XDFR_MRI_ACC_L3)
...
cor.test(averages_from_template[["low_incorrect"]]$V11,temp_plot_data$omnibus_span_no_DFR_MRI)
...
cor.test(averages_from_template[["low_load_correct_diff"]]$V11,temp_plot_data$XDFR_MRI_ACC_L3)
