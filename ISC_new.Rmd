...
```{python load in numpy data, eval=FALSE, include=FALSE}
...
```{r load in correlation data, eval=FALSE, include=FALSE}
...
fusiform_sISC_ordered <- high_correct_sISC_fusiform[span_order,span_order,]
DFR_sISC_ordered <- high_correct_sISC_DFR[span_order,span_order,]

# select out only subjects who were included in group analyses 

fusiform_sISC_ordered_group <- fusiform_sISC_ordered[c(1:56,58:113,115:170),c(1:56,58:113,115:170),]
DFR_sISC_ordered_group <- DFR_sISC_ordered[c(1:56,58:113,115:170),c(1:56,58:113,115:170),]

# remove NaNs
fusiform_sISC_ordered_group[is.nan(fusiform_sISC_ordered_group)] <- NA
DFR_sISC_ordered_group[is.nan(DFR_sISC_ordered_group)] <- NA
...
avg_sISC_fusiform <- avg_ISC(high_correct_sISC_fusiform)
avg_sISC_DFR <- avg_ISC(high_correct_sISC_DFR)

overall_avg_sISC_fusiform <- rowMeans(avg_sISC_fusiform)
overall_avg_sISC_DFR <- rowMeans(avg_sISC_DFR)
...
for (TR in seq.int(1,14)){
  data <- data.frame(fusiform_sISC_ordered_group[,,TR])
...
z_trans_fusiform <-  atanh(fusiform_sISC_ordered_group)
...
data_to_plot <- cbind(data_to_plot,overall_avg_sISC_fusiform,overall_avg_sISC_DFR)

cor.test(overall_avg_sISC_fusiform,data_to_plot$omnibus_span_no_DFR_MRI)
cor.test(overall_avg_sISC_fusiform,data_to_plot$XDFR_MRI_ACC_L3)
cor.test(overall_avg_sISC_fusiform,data_to_plot$WHO_ST_S32)
cor.test(overall_avg_sISC_fusiform,data_to_plot$BPRS_TOT)

ggplot(data=data_to_plot,aes(x=overall_avg_sISC_fusiform,omnibus_span_no_DFR_MRI))+
  geom_point()+
  stat_smooth(method="lm")+
  ggtitle("Avg ISC fusiform vs omnibus span")

ggplot(data=data_to_plot,aes(x=overall_avg_sISC_fusiform,XDFR_MRI_ACC_L3))+
  geom_point()+
  stat_smooth(method="lm")+
  ggtitle("Avg ISC fusiform vs L3 DFR Acc")

ggplot(data=data_to_plot,aes(x=overall_avg_sISC_fusiform,WHO_ST_S32))+
  geom_point()+
  stat_smooth(method="lm")+
  ggtitle("Avg ISC fusiform vs WHODAS")

ggplot(data=data_to_plot,aes(x=overall_avg_sISC_fusiform,BPRS_TOT))+
  geom_point()+
  stat_smooth(method="lm")+
...
corr_ISC(avg_sISC_fusiform,data_to_plot[,c(1,7)])
corr_ISC(avg_sISC_fusiform,data_to_plot[,c(1,8)])
corr_ISC(avg_sISC_fusiform,data_to_plot[,c(1,14)])
corr_ISC(avg_sISC_fusiform,data_to_plot[,c(1,20)])
...
for (TR in seq.int(1,14)){
  data <- data.frame(DFR_sISC_ordered_group[,,TR])
  rownames(data) <- c(1:168)
  colnames(data) <- c(1:168)
  data %>%
...
z_trans_DFR <-  atanh(DFR_sISC_ordered_group)
...
cor.test(overall_avg_sISC_DFR,data_to_plot$omnibus_span_no_DFR_MRI)
cor.test(overall_avg_sISC_DFR,data_to_plot$XDFR_MRI_ACC_L3)
cor.test(overall_avg_sISC_DFR,data_to_plot$WHO_ST_S32)
cor.test(overall_avg_sISC_DFR,data_to_plot$BPRS_TOT)

ggplot(data=data_to_plot,aes(x=overall_avg_sISC_DFR,omnibus_span_no_DFR_MRI))+
  geom_point()+
  stat_smooth(method="lm")+
  ggtitle("Avg ISC DFR vs omnibus span")

ggplot(data=data_to_plot,aes(x=overall_avg_sISC_DFR,XDFR_MRI_ACC_L3))+
  geom_point()+
  stat_smooth(method="lm")+
  ggtitle("Avg ISC DFR vs L3 DFR Acc")

ggplot(data=data_to_plot,aes(x=overall_avg_sISC_DFR,WHO_ST_S32))+
  geom_point()+
  stat_smooth(method="lm")+
  ggtitle("Avg ISC DFR vs WHODAS")

ggplot(data=data_to_plot,aes(x=overall_avg_sISC_DFR,BPRS_TOT))+
  geom_point()+
  stat_smooth(method="lm")+
...
corr_ISC(avg_sISC_DFR,data_to_plot[,c(1,7)])
corr_ISC(avg_sISC_DFR,data_to_plot[,c(1,8)])
corr_ISC(avg_sISC_DFR,data_to_plot[,c(1,14)])
corr_ISC(avg_sISC_DFR,data_to_plot[,c(1,20)])
