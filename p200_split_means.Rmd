...
```{r split into groups based on capacity}
...
```{r playing with regression, include = FALSE, eval=FALSE }
all_data <- merge(p200_clinical_zscores, p200_indiv_ROI_DFR_delay)
all_data <- merge(all_data,p200_cog_zscores)
all_data <- merge(all_data, p200_indiv_ROI_DFR_cue)
all_data  <- merge(all_data, p200_all_RS)
all_data <- merge(all_data, construct_vars_omnibus)
all_data <- merge(all_data, p200_beta_conn_cue)
all_data <- merge(all_data, p200_demographics)
all_data <- merge(all_data,p200_HPC_Post)
all_data  <- merge(all_data,p200_beta_conn_delay,by="PTID") 
all_data$omnibus_c <- c(scale(all_data$omnibus_span_no_DFR_MRI, center=TRUE,scale=FALSE))
all_data$acc_L3_c <- c(scale(all_data$XDFR_MRI_ACC_L3, center=TRUE,scale=FALSE))
all_data$BPRS_c <- c(scale(all_data$BPRS_TOT, center=TRUE,scale=FALSE))
all_data$DFR_ROIs_c <- c(scale(all_data$DFR_ROIs, center=TRUE,scale=FALSE))
all_data$DFR_sq_c <- all_data$DFR_ROIs_c^2
all_data$FPCN_HCP_delay_L3_c <- c(scale(all_data$FPCN_HPC_L3.y,center=TRUE,scale=FALSE))


mod <- lm(XDFR_MRI_ACC_L3 ~ omnibus_span_no_DFR_MRI + DFR_ROIs + DFR_sq + GENDER + AGE + SCANNER + BPRS_TOT + FPCN_HPC_L3.x + FPCN_HPC_L3.y + FPCN_FPCN_L3.x + FPCN_FPCN_L3.y + FPCN_FPCN + FPCN_DMN +  omnibus_span_no_DFR_MRI*BPRS_TOT + omnibus_span_no_DFR_MRI*DFR_ROIs + BPRS_TOT*DFR_ROIs, data = all_data)
summary(mod)

mod2 <- lm(XDFR_MRI_ACC_L3 ~ omnibus_span_no_DFR_MRI + DFR_ROIs + DFR_sq + GENDER + AGE + SCANNER + BPRS_TOT +  omnibus_span_no_DFR_MRI*BPRS_TOT + omnibus_span_no_DFR_MRI*DFR_ROIs + BPRS_TOT*DFR_ROIs, data = all_data)
summary(mod2)


mod4 <- lm(BPRS_TOT ~ omnibus_span_no_DFR_MRI + XDFR_MRI_ACC_L3+ DFR_ROIs + GENDER + AGE + SCANNER + omnibus_span_no_DFR_MRI*XDFR_MRI_ACC_L3 + omnibus_span_no_DFR_MRI*DFR_ROIs, data = all_data)
summary(mod4)

mod4 <- lm(BPRS_TOT ~ omnibus_c + acc_L3_c + omnibus_c*acc_L3_c + DFR_ROIs + DFR_sq + FPCN_DMN + SCANNER + GENDER + AGE, data = all_data)
summary(mod4)

mod5 <- lm(omnibus_span_no_DFR_MRI ~ acc_L3_c + BPRS_c + DFR_sq + SCANNER + GENDER + AGE,data=all_data)
summary(mod5)

mod3 <- lm(XDFR_MRI_ACC_L3 ~ omnibus_c+ BPRS_c + GENDER + AGE + SCANNER  +FPCN_HCP_delay_L3_c + omnibus_c*BPRS_c, data = all_data)
summary(mod3)

```


```{r using span as moderator}
