...
clf_acc <- read.csv('data/MVPA/fusiform_csvs/clf_acc.csv')
best_c <- read.csv('data/MVPA/fusiform_csvs/best_C.csv')

# averaages from template 
averages_from_template <- list(high_correct = read.csv('data/MVPA/fusiform_csvs/all_suj_high_correct_avg.csv',header=FALSE), 
                               high_incorrect = read.csv('data/MVPA/fusiform_csvs/all_suj_high_incorrect_avg.csv',header=FALSE), 
                               low_correct = read.csv('data/MVPA/fusiform_csvs/all_suj_low_correct_avg.csv',header=FALSE), 
                               low_incorrect = read.csv('data/MVPA/fusiform_csvs/all_suj_low_incorrect_avg.csv',header=FALSE))
...
individual_trial_averages_probs <- list(
  high_correct = read.csv('data/MVPA/fusiform_csvs/all_suj_high_correct_indiv_trial_avg_probs.csv',header=FALSE),
  high_incorrect = read.csv('data/MVPA/fusiform_csvs/all_suj_high_incorrect_indiv_trial_avg_probs.csv',header=FALSE),
  low_correct = read.csv('data/MVPA/fusiform_csvs/all_suj_low_correct_indiv_trial_avg_probs.csv',header=FALSE),
  low_incorrect = read.csv('data/MVPA/fusiform_csvs/all_suj_low_incorrect_indiv_trial_avg_probs.csv',header=FALSE)) 
...
individual_trial_averages_preds <- list(
  high_correct = read.csv('data/MVPA/fusiform_csvs/all_suj_high_correct_indiv_trial_avg_preds.csv',header=FALSE),
  high_incorrect = read.csv('data/MVPA/fusiform_csvs/all_suj_high_incorrect_indiv_trial_avg_preds.csv',header=FALSE),
  low_correct = read.csv('data/MVPA/fusiform_csvs/all_suj_low_correct_indiv_trial_avg_preds.csv',header=FALSE),
  low_incorrect = read.csv('data/MVPA/fusiform_csvs/all_suj_low_incorrect_indiv_trial_avg_preds.csv',header=FALSE)) 
...
save(list=c("clf_acc", "best_c", "averages_from_template", "individual_trial_averages_probs","individual_trial_averages_preds"), file = "data/MVPA_fusiform.RData")
...
In the template, however, there is no difference between load at encoding. Instead, we see that the low correct have significantly more likelihood of having a face classified at probe than either of the high load templates. 
...
Weirdly, in the template, it is during the low load trials where we really can see prediction from the faces. 
...
Here, we see that there are significant differences in the high incorrect trials during encoding - medium capacity subjects have increased face representation than high capacity subjects. This is the same inverted U shaped relationship that we see in the activity patterns. We also see the same pattern in the correct high load trials, but it only trends towards significance. 

There are no differences during probe. 
...
There are no significant differences during encoding, but we do see significantly greater likelihood of face classification at TR 12 in low capacity subjects than both medium and high capacity subjects. 
...
The only significant relationship between classification probability averaged over time and behavior is with BPRS Total in the low load correct trials. As there is more probability of face classification, there tends to be higher levels of psychiatric symptoms. 
...
cor.test(indiv_avg_over_time$high_correct, indiv_avg_over_time$BPRS_TOT)
...
We see that there is a significant negative relationship between probability of face classification at delay in correct low load trials and high load accuracy and the difference between correct and incorrect trials at low load. 
...
Probability of classification at correct low load trial is significantly correlated with omnibus span at TR 12 and positively with BPRS at TR 10. We also see a significant negative relationship with accuracy in incorrect high load trials. Here, we're looking at the scatter plots from TR 11. At TR 11, we see a significant relationshihp between the correct - incorrect difference at high load high load accuracy. 
...
cor.test(individual_trial_averages_probs[["low_correct"]]$V10,temp_plot_data$BPRS_TOT)
...
There is a significant relationship between span and classification probability in incorrect high load trials - the higher the probability of face classification, the higher the capacity.  
...
There is a significant negative relationship with accuracy and classification at incorrect high load trials and the difference between high correct and incorrect trials. 
