...
#save(list=c("fMRI_PTID", "CDA","CDA_fMRI","ERPS_times_DFR", "ERSPS_times_DFR", "ERSPS_freqs", "ERPS_midOccip_DFR","ERSPS_midOccip_DFR", "ERPS_Oavg_DFR","ERSPS_Oz_DFR","ERPS_Pz_DFR", "ERPS_Pavg_DFR"), file="data/EEG_for_Will.RData")
...
```{r scatter plots span vs ERP high load measures}
...
```{r scatter plots span vs ERP load effect measures}
...
```{r check if values different from zero}
...
```{r find spectra where indiv diffs corr with span squared}
...
```{r plot frequency spectrum corr to omnibus span squared}
...
```{r plot frequency spectrum corr to omnibus span squared - thresholded}
...
```{r select out times with significant load effects - alpha}
...
```{r select out cue delay probe from low_alpha}
...
```{r select out cue delay probe from high_alpha}
...
```{r prep low_alpha for bar plots}
...
```{r examine linear correlation for low alpha high load}
...
```{r plot split group low_alpha - load effects}
...
```{r run anovas for low_alpha - load effect}
...
```{r prep high_alpha for bar plots}
...
```{r examine linear correlation for high alpha high load}
...
```{r plot split group high_alpha - load effects}
...
```{r run anovas for high_alpha - load effect}
...
``` 
```{r select out times with significant load effects - beta}
...
```{r examine linear correlation for beta high load}
...
```{r select out times with significant load effects - theta}
...
```{r correlate span to theta load effects}
...
```{r examine linear correlation for theta high load}
...
```{r check for relationship between theta and MRI load effects}
...
```{r select out times with significant load effects - low_gamma}
...
```{r examine linear correlation for low gamma high load}
...
```{r compare span groups with EEG cap vs omnibus span}
...
```
```{r check change in span group between EEG capacity and omnibus span}
...
```{r look at PCs when boundary cases removed}
