...
reg_data$GROUP <- reg_data$GROUP - 1
...
for (fold in seq.int(1,10)){ 
  ...
  train_data <- train_data[,c(2, 5:71)]
  test_data <- assessment(split$splits[[fold]])
  test_data <- test_data[,c(2, 5:71)]
  train_data[,1:64] <- sapply(train_data[,1:64],scale)
  train_data <- data.frame(train_data)
  test_data[,1:64] <- sapply(test_data[,1:64], scale)
...
When comparing models, ElasticNet is our best model, explaining ~9% of the variance. 
...
for (fold in seq.int(1,10)){ 
  ...
  # set up training and test set for fold
  train_data <-  analysis(split$splits[[fold]])
  train_data <- train_data[,c(4, 5:71)]
  test_data <- assessment(split$splits[[fold]])
  test_data <- test_data[,c(4, 5:71)]
  train_data[,1:64] <- sapply(train_data[,1:64],scale)
  train_data <- data.frame(train_data)
  test_data[,1:64] <- sapply(test_data[,1:64], scale)
  test_data <- data.frame(test_data)
...
Again, ElasticNet is our best model, explaining 15% of the variance with the lowest RMSE. 
...
for (fold in seq.int(1,10)){ 
  ...
  # set up training and test set for fold
  train_data <-  analysis(split$splits[[fold]])
  train_data <- train_data[,c(3, 5:71)]
  test_data <- assessment(split$splits[[fold]])
  test_data <- test_data[,c(3, 5:71)]
  train_data[,1:64] <- sapply(train_data[,1:64],scale)
  train_data <- data.frame(train_data)
  test_data[,1:64] <- sapply(test_data[,1:64], scale)
  test_data <- data.frame(test_data)
...
compare_high_acc$stepwise[2] <- cor(cv_preds_high_acc$true, cv_preds_high_acc$pred)^2
...
Again, ElasticNet is our best model. Here, we explain 12% of the variance. 
