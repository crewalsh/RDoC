...
cumulative_df_ERP <- data.frame(matrix(nrow=1,ncol=2))
...
for (ERP in seq.int(25,36)){
  for (fMRI in seq.int(2,23)){
    ...
    if (cor_results$p.value < 0.05){
      ...
      cumulative_df_ERP <- rbind(cumulative_df_ERP, c(colnames(ERP_fMRI)[ERP], colnames(ERP_fMRI)[fMRI]))
    } 
...
cumulative_df_ERSP <- data.frame(matrix(nrow=1,ncol=2))
...
for (ERSP in seq.int(25,60)){
  for (fMRI in seq.int(2,23)){
    ...
    if (cor_results$p.value < 0.05){
      ...
      sig_plot_count <- sig_plot_count +1 
      cumulative_df_ERSP <- rbind(cumulative_df_ERSP, c(colnames(ERSP_fMRI)[ERSP], colnames(ERSP_fMRI)[fMRI]))
